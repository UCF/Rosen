...
Webcom.events = {
	get_month_from_rss: function(str) {
		var months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
		var index  = Number(str.substr(5, 2));
		return months[index - 1];
	},
	get_day_from_rss: function(str) {
		return str.substr(8, 2);
	},
	callback: function($, _this){
		...
		$.getUCFEvents({
			...
			for (var i = 0; i < data.length; i++){
				...
				title.text(e.title);
				day.text(Webcom.events.get_day_from_rss(e.starts));
				month.text(Webcom.events.get_month_from_rss(e.starts));
